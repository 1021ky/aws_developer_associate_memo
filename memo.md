...
* DynamoDBの操作はHTTPベースのAPIで行う
  * Table API
    ...
    * Scan
      * テーブルを総なめする
      * 最大1MBのデータを取得可能
      * マルチスレッド処理をサポートするプログラミング言語で並列でScanオペレーションを実行することで並列スキャンを実行できる。高速。
        * パラメータとしてSegmentとTotalSegmentsを指定する
        * Segment スキャンされるセグメント 各スレッドがスキャンするセグメント
        * TotalSegments並列スキャンのセグメントの合計数 アプリケーションのスレッド数
...
* Scaling
  ...
  * Global Secondary Index（GSI）
    ...
    * Partation Keyをまたいで検索を行うためのインデックス
      * Partition Key以外をプライマリキーとして使いたいときに使う
...
* オプティミスティック（optimistic）とは楽観的の意味
  * 反対語はペシミスティック（pessimistic）悲観的
  * 悲観的ロックはテーブルにロックを掛けてから更新する排他制御
    * 他にも更新するクライアントがいて、かち合うことが頻繁にあること前提ということで悲観的
  * 楽観的ロックはロックはテーブルに掛けずに更新する排他制御のこと
    * 他にも更新するクライアントがいるが、頻繁にかちあわない前提ということで楽観的
* データベースの書き込みが他のユーザーの書き込みによって上書きされないように保護する方法
* オプティミスティックロックを使用するためには、バージョン番号として機能する属性が必要。
* 項目をテーブルから取り出すと、アプリケーションはその項目のバージョン番号を記録
* サーバー側のバージョン番号が変わっていないときのみ更新する
...
* 利用ケース
  * バッファリング
    ...
    * キューがなかったらコンシューマはリクエストスパイクで高負荷になる
  * ワークキュー
...
* AWSクラウド上のロードバランシングサービス
  ...
  * 同一インスタンスで複数ポートに負荷分散
    * ターゲットグループに割り当てる際に複数ポートを個別のターゲットとして登録することが可能
    * 1つのインスタンスに対して、複数ポートで負荷分散が可能
    * コンテナを用いる際には同一インスタンス上で複数のコンテナ（サーバー）を運用する必要があるので有用
    * ECSをターゲットとして利用する場合は、動的ポートマッピング機能を利用できる
      * ECSのタスク（サービス）をターゲットグループに登録することでECSの各タスクに対して負荷分散を実施できる
      * 各タスクに割り当てられたECSクラスタ上のポートをターゲットグループに自動的に登録
        * NLBにはSecurity Groupが設定できないためコンテナインスタンス側で設定する
          * ECSタスクに動的に設定されるポートの範囲を意識する必要がある
          * /proc/sys/net/ipv4/ip_local_port_rangeで設定
  * コネクションタイムアウト
    * 無通信状態が続くとコネクションを自動で切断
      * ALB/CLBはデフォルトは60秒で、1〜4000病で自由に設定可能
      * NLBのデフォルトは350秒で固定
  * Connection Draining（登録解除の遅延）
    * バックエンドのEC2インスタンスをELBから登録解除したり、ヘルスチェックが失敗したときに、新規リクエストの割り振りは中止して、処理中のリクエストは終わるまで一定期間待つ
    * すべてのELBでデフォルトで有効、タイムアウト300秒
    * タイムアウト最大3600秒
  * スティッキーセッション（stickiness）
    * 同じユーザからきたリクエストをすべて同じEC2インスタンスに送信
      * アプリケーションでのセッション情報、父児ファイルなどをEC2インスタンスが保持する構成の場合に必要
        * 保持期限はELBで指定できる
          * 無制限にすることもできるが、ブラウザを閉じれば終了する
      * デフォルトで無効
      * HTTP/HTTPSでのみ利用可能
      * セッション情報などは別のDBサーバやキャッシュサーバに持たせるのが望ましい。
* Layer7のHTTP,HTTPSにはApplication Load Balancer（ALB）
  * 任意のSecurity Groupを指定可能
  * ターゲットにLambda関数を登録可能
  * Websocketに対応
  * コンテンツベースのルーティング
    * パスベースのルーティング
  ...
  * ルーティングのアクション
    * 転送アクション：指定されたターゲットグループにリクエストを転送
    * リダイレクト：ホストにルーティング
    * 固定レスポンス：クライアントリクエストを破棄し、2XX,4XX,5XXなどのカスタムのHTTPレスポンスを返す
    * Cognito認証アクション：[HTTPSリスナーのみ]Amazon Cognitoを使用してユーザー認証を実施
    * OIDC認証アクション：[HTTPSリスナーのみ]OpenID Connect（OIDC）に準拠するIDプロバイダーを使用してユーザー認証を実施
  * ALB利用時のクライアントのIPアドレス取得
    * X-Forwarded-Forヘッダーを使用する
    * NLBの場合は何も設定せずに透過的にクライアントIPアドレスを取得可能
* Layer4のTCP（1-65535番号ポート）, TLS, UDPにはNetwork Load Balancer（NLB）
  * Security Groupの指定はできない
    * TargetのSecurity GroupでクライアントIPの接続を許可する必要がある
  * Internet-facing、InternalともにIPアドレスが固定
    * NLB作成時に自動割当されたIPアドレス、またはNLB作成時に指定した自分が持っているElastic IPのいずれか
* Classic Load Balancer（CLB）
  * HTTP/HTTPS/TCP用
  * EC2-Classicネットワーク用ロードバランサーだが、EC2-Classicは終了になるので、もう気にしなくていいかも
* NLB自体もキャパシティが自動で増加（スケールアップ、スケールアウト）する
  * ALB/CLBはスケールアウトしたときにIPアドレスが変わるのでDNS名で参照するのが基本
* Global Acceleratorと連携するとALBのIPを固定にできる
